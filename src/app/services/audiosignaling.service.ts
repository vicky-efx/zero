...
export class AudiosignalingService {
  ...
  async createAudioRoom(peer: RTCPeerConnection): Promise<string> {
    const roomId = `audio-room-${Date.now()}`;
    const roomRef = doc(this.firestore, 'audioRooms', roomId);
  ...
  async joinAudioRoom(roomId: string, peer: RTCPeerConnection): Promise<void> {
    const roomRef = doc(this.firestore, 'audioRooms', roomId);
